...
function ProfileEdit() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        if (userId) {
          const response = await axios.get(`https://server-0xgn.onrender.com${userId}`
          );
  ...
  const handleupdate = async () => {
    try {
      ...
      const response = await axios.patch(
        `https://server-0xgn.onrender.com${userId}` || `https://server-0xgn.onrender.com${userId}`,
      ...
      {userData && (
        <div className="w-80 h-1/2 mx-4 bg-slate-100 shadow-lg rounded-lg hover:shadow-xl transition duration-500">
          ...
          <img
            ...
            src={`https://server-0xgn.onrender.com${userData.profilePic}`}
